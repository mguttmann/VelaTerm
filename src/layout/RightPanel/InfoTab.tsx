//! Right-side Info tab: basic session details and uptime, Claude/Codex/Grok quotas,
...
export function InfoTab({ session, cwd }: { session: Session; cwd: string | null }) {
  ...
  return (
    <div className="info-sections">
      ...
      <Section
        id="agent"
        title={isAgent ? "Agent" : "Process"}
        tag={isAgent ? runtime?.agent || session.kind : (runtime?.pid ?? "—")}
      >
      ...
      {hasContext && <TurnSection key={session.id} session={session} agentState={agentState} currentTool={currentTool} />}
