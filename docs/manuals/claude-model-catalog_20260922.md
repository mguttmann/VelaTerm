...
Offering and accepting differ on purpose. The menus offer only what the installed CLI lists, but the CLI's list is a shortlist of what it offers, not everything it accepts. A knowledge-base or security job saved with a model the list no longer names (for example `claude-opus-5` after Opus 5.5 became the default, or Opus 5.5 after the next release) therefore still validates: accepted are the list, the bundled table and the stored identifier itself when it is a well-formed Claude model id (`claude-` followed by lowercase letters, digits, dots and hyphens, optionally `[1m]`, at most 64 characters). The CLI decides at start whether it still runs that model.
