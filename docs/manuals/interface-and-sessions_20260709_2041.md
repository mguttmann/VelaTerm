...
- **Sort by activity**: an optional mode in the sidebar filter menu (its own "Order" section, also under Settings → Behavior). While it is on, the most recently active projects and sessions float to the top: a session counts as active when its agent starts working, asks a question or waits for you, when you open it or switch to its tab or pane, or when you send it input. Things that happen without you do not count: the layout restored at startup, the tab that takes over after you close one, and visitors of a shared session. Groups take the activity of their contents; items without recorded activity keep their manual order below the active ones. Activity is remembered across restarts, also while the mode is off, so switching it on later shows the real history. In this mode you cannot drag rows to reorder them (moving a session into a group, a project or under another session still works); switch the mode off and the manual order returns unchanged.
